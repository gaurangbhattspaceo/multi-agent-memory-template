...
exec /app/tools/safe-exec.sh git commit [worktree] -m "fix: [address review feedback]"
exec /app/tools/safe-exec.sh git push [worktree] origin [branch]
sessions_send(agentId: "lead-agent", message: "Fixed review comments on MR #[number]. Ready for re-review.")
```
...
```
sessions_send(agentId: "lead-agent", message: "RE: MR #[number] comment on [file:line] — Can you clarify? Did you mean [interpretation A] or [interpretation B]?")
```
...
```
sessions_send(agentId: "lead-agent", message: "RE: MR #[number] comment on [file:line] — I think the current implementation is correct because:
...
- Feedback identifies DANGER-ZONES violations
